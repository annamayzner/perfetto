...
export function renderCharts(app: App): m.Children {
  return [
    ...
    // FlameChartSimple section
    m('h2', {style: {marginTop: '32px'}}, 'FlameChartSimple'),
    renderWidgetShowcase({
      renderWidget: (opts) => {
      ...
      renderWidget: (opts) => {
        return m(ProportionBar, {
          ...
          segments: [
            ...
            // Zero-weight: dropped from the bar, still listed in the legend.
            {label: 'Other', weight: 0, value: '0 B'},
          ],
        });
      },
      initialOpts: {
  ...
  return {
    view: ({attrs}) => {
      ...
      const config: HistogramLoaderConfig = {
        ...
        filter:
          isFilter && brushedRange !== undefined
            ? {min: brushedRange.start, max: brushedRange.end}
            : undefined,
      };
      ...
      return m('div', [
        ...
        m(
          ...
          [
            ...
            brushedRange
              ? `\n\nBrushed: [${brushedRange.start.toFixed(2)}, ${brushedRange.end.toFixed(2)}]`
              : attrs.brushMode !== 'off'
...
// Static sample data for the FlameChartSimple demo: a synthetic CPU profile.
const FLAMECHART_SAMPLE_DATA: FlameChartSegment = {
  name: 'Total',
  value: 1000,
  cssColor: '#5f6368',
  children: [
