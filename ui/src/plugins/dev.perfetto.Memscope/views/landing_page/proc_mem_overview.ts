  ...
  implements m.ClassComponent<ProcessMemDetailsAttrs>
{
  ...
  view({attrs}: m.Vnode<ProcessMemDetailsAttrs>) {
    ...
    // only the capture strip waits on the capture query. Both tab bodies stay
    // mounted and are toggled with a Gate (display:none when hidden) rather
    // than conditionally rendered, so switching tabs doesn't remount the
    // section components and re-run their data loads.
    return [
      error && m('p.pf-error', `Error: ${error}`),
      !error &&
        capture !== undefined &&
        this.renderCaptureStrip(trace, capture),
  ...
  private renderCaptureStrip(trace: Trace, info: CaptureInfo): m.Children {
    ...
    const sources: {key: string; label: string; facts?: string}[] = [
      {key: 'smaps', label: 'smaps', facts: sourceFacts(info.smaps)},
      {
        key: 'dump',
        label: 'java_hprof',
        facts: info.dumps > 0 ? `${info.dumps} dumps` : undefined,
      },
      {key: 'native', label: 'heapprofd', facts: sourceFacts(info.native)},
    ];
