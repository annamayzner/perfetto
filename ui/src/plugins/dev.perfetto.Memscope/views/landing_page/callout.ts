...
// falls back to an intent-specific default so the landing-page sections can
// omit `icon` and still get a sensible glyph.
...
// A compact callout strip: a leading icon plus a one-line message, tinted by
// intent. Used by the landing-page sections to surface the single most
// relevant takeaway (or caveat) alongside the figures. Mirrors the interface
// and colour treatment of the shared `widgets/callout` Callout, with a
// Memscope-specific geometry and intent-based default icons.
