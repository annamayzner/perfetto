...
import {
  FlamegraphChart,
  type FlameChartSegment,
} from '../../../../../components/widgets/charts/flamegraph';
...
// Extras for the selected snapshot and (in compare mode) the baseline, loaded
// in their own slot keyed by the selection so picking a snapshot doesn't
// re-scan all the smaps snapshots.
interface MemExtrasPair {
  readonly sel: MemExtras;
  readonly base?: MemExtras;
}
...
export class MemoryMap implements m.ClassComponent<MemoryMapAttrs> {
  ...
  // Cross-source extras for the selected/baseline snapshot (keyed by selection
  // so it's a small, fast query when the snapshot changes).
  private readonly extrasSlot = new QuerySlot<MemExtrasPair>();

  onremove() {
    this.slot.dispose();
    this.extrasSlot.dispose();
  }
  ...
  view({attrs}: m.Vnode<MemoryMapAttrs>): m.Children {
    ...
    // Extras for the resolved snapshot(s) — loaded in their own slot.
    const extras = this.extrasSlot.use({
      key: {
      ...
      queryFn: async () => ({
        sel: await loadExtras(trace, upid, snap.ts),
        base:
          base !== undefined
            ? await loadExtras(trace, upid, base.ts)
            : undefined,
      }),
    }).data;
    const selExtras = extras?.sel ?? {nativeSeen: 0, javaReachable: 0};
    ...
    const baseByLabel =
      base !== undefined
        ? flattenMemTree(
            buildMemTree(
              base,
              extras?.base ?? {nativeSeen: 0, javaReachable: 0},
            ),
            new Map(),
          )
        : undefined;
    const tree = buildMemTree(snap, selExtras, baseByLabel);
